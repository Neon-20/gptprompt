...
const Form = ({type , post , setPost , submitting, handleSubmit}) => {
  ...
  <p className="desc text-left max-w-md">
    ...
    and let your imagination run wild with an AI powered
    platform.
  </p>
  ...
  <label>
    <span className="font-satoshi font-semibold text-base
    text-gray-700"> Tag {` `}
    <span className="font-normal">(#product,#webdevelopment,#ideas,#creativity)</span>
    </span>
    <input
    value = {post.tag}
    onChange={(e)=> setPost({...post,
  ...
  <Link href="/" className="px-5 py-1.5 text-sm bg-gray-900
  rounded-full text-white text-md  hover:bg-green-100">
  cancel</Link>

  <button type="submit"
  disabled={submitting}
  className="px-5 py-1.5 text-sm bg-primary-orange
  rounded-full text-white hover:bg-indigo-700"
  >
